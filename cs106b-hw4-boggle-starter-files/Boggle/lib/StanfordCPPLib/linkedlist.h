 ...
 * function to the STL <code>LinkedList</code> type, but is simpler both
...
public:
     ...
     * Usage: LinkedList<ValueType> vec;
     *        LinkedList<ValueType> vec(n, value);
     * ---------------------------------------
     * Initializes a new LinkedList.  The default constructor creates an
     * empty LinkedList.  The second form creates an array with <code>n</code>
     ...
     */
    LinkedList();
    explicit LinkedList(int n, ValueType value = ValueType());
    LinkedList(const std::list<ValueType>& v);
     ...
     *</pre>
     */
    LinkedList& operator+=(const LinkedList& v2);
    LinkedList& operator+=(const ValueType & value);

    bool operator==(const LinkedList& v2);
    bool operator!=(const LinkedList& v2);
...
//        iterator(const LinkedList *vp, int index) {
//            this->vp = vp;
//            this->index = index;
//        }
...
//        bool operator<(const iterator & rhs) {
//            if (vp != rhs.vp) error("LinkedList Iterator::operator <: Iterators are in different LinkedLists");
...
template <typename ValueType>
LinkedList<ValueType>::LinkedList(int n, ValueType value) {
    for (int i = 0; i < n; i++) {
        add(value);
    }
}
...
/*
 * Implementation notes: LinkedList methods
 * ------------------------------------
 * The basic LinkedList methods are straightforward and should require
...
template <typename ValueType>
void LinkedList<ValueType>::push_back(ValueType value) {
    m_elements.push_back(value);
}
...
template <typename ValueType>
LinkedList<ValueType> LinkedList<ValueType>::operator+(const LinkedList& list2) const {
    LinkedList<ValueType> list = *this;
    for (ValueType value : list2) {
        list.add(value);
    }
    return list;
}
...
template <typename ValueType>
LinkedList<ValueType> & LinkedList<ValueType>::operator+=(const LinkedList& list2) {
    for (ValueType value : list2) {
        *this += value;
    }
    return *this;
...
template <typename ValueType>
LinkedList<ValueType> & LinkedList<ValueType>::operator+=(const ValueType & value) {
    this->add(value);
    return *this;
}
...
bool LinkedList<ValueType>::operator==(const LinkedList& list2) {
    ...
    auto itr1 = this->begin(), end1 = this->end,
...
template <typename ValueType>
LinkedList<ValueType>& LinkedList<ValueType>::operator=(const LinkedList& src) {
    if (this != &src) {
        deepCopy(src);
    }
    return *this;
