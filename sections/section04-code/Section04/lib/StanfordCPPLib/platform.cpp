...
bool Platform::regexMatch(string s, string regexp) {
    ostringstream os;
    os << "Regex.match(";
...
int Platform::regexMatchCount(string s, string regexp) {
    ostringstream os;
    os << "Regex.matchCount(";
    writeQuotedString(os, urlEncode(s));
    os << ",";
...
int Platform::regexMatchCountWithLines(string s, string regexp, string& linesOut) {
    ostringstream os;
...
string Platform::regexReplace(string s, string regexp, string replacement, int /* limit */) {
    ostringstream os;
    os << "Regex.replace(";
    writeQuotedString(os, urlEncode(s));
    os << ",";
...
void Platform::setExitOnClose(const GWindow & gw, bool value) {
    ostringstream os;
...
void Platform::setWindowSize(const GWindow & gw, int width, int height) {
    ostringstream os;
 ...
 * pass (-1, -1) to center
 */
void Platform::setWindowLocation(const GWindow & gw, int x, int y) {
    ostringstream os;
...
bool Platform::isEnabled(GObject* gint) {
    ostringstream os;
...
void Platform::setEnabled(GObject* gint, bool value) {
    ostringstream os;
    os << "GInteractor.setEnabled(\"" << gint << "\", " << boolalpha << value << ")";
    putPipe(os.str());
}

void Platform::setAntialiasing(bool value) {
    ostringstream os;
...
void Platform::drawInBackground(const GWindow & gw, const GObject *gobj) {
    ostringstream os;
...
void Platform::exitGraphics(bool abortBlockedConsoleIO) {
    if (abortBlockedConsoleIO && isBlockedForConsoleIO()) {
...
int startupMain(int argc, char **argv) {
    ...
    getPlatform()->setCppLibraryVersion();
...
static string getPipe() {
    ...
    while (charsRead < charsReadMax) {
        ...
        if (readFileResult == 0) break;   // failed to read from subprocess
...
static string getResult(bool consumeAcks) {
    while (true) {
        ...
        if (((result || event) && hasACMException) ||
                (!result && !event && (hasException || hasError))) {
            ...
            if (line.find("\tat ") == 0 || line.find("   at ") != string::npos) {
...
void Platform::setCppLibraryVersion() {
    ostringstream out;
...
string Platform::getCppLibraryVersion() {
    return STANFORD_CPP_LIB_VERSION;
}

string Platform::getJavaBackEndVersion() {
...
void Platform::setConsoleExitOnClose(bool value) {
    ostringstream os;
