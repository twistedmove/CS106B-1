...
const ValueType & Vector<ValueType>::get(int index) const {
    if (index < 0 || index >= count) error("Vector::get: index out of range");
    return elements[index];
}

template <typename ValueType>
void Vector<ValueType>::set(int index, const ValueType & value) {
    if (index < 0 || index >= count) error("Vector::set: index out of range");
...
void Vector<ValueType>::remove(int index) {
    if (index < 0 || index >= count) error("Vector::remove: index out of range");
...
Vector<ValueType> Vector<ValueType>::operator+(const Vector & v2) const {
    ...
    __foreach__ (ValueType value __in__ v2) {
        vec.add(value);
    }
    return vec;
}
